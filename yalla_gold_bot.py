...
try:
    ...
    # 🔥 إصلاح العلق: إذا صفقة فات عليها 45 دقيقة - اقفلها
    ...
    if trade:
        ...
        if (typ=="BUY" and price<=sl) or (typ=="SELL" and price>=sl):
            ...
            save("daily.json", daily)
            msg = f"🏆 TP {typ} +{pips:.1f} 🔥 | {now} | +{p001:.2f}$"
            if os.path.exists("trade.json"): os.remove("trade.json")
        else:
            msg = f"🔄 {typ} مفتوحة {pips:+.1f} | {now} | {price:.2f}$ | لا نرسل جديدة حتى تخلص"
    else:
        # ما كاينش صفقة - ارسل سكالب فورا
        entry = round(price,2)
        ...
        msg = f"""━━━━━━━━━━━━━━━━━━━━
⚡ SCALP SIGNAL - {typ} {score_abs}% {'⏰ 30د' if must_send else ''}
━━━━━━━━━━━━━━━━━━━━
⏰ {now} | V12.1 كل 30دقيقة
💵 دخول: {entry}$ | {bias}
